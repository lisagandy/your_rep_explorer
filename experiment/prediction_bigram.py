...
modelDir = '/Users/lisagandy/Desktop/classif_bigram/'
...
def writeFileBigram(descript,lsAllWords,fTrain,classif):
    descript = pyU.removePunctuation(descript)
    descript = pyU.sStripStopWordsAll(descript)
    fTrain.write(str(classif)+' ')
    #print descript
    ...
    for index in range(1,len(words)-1):
        ...
        word1 = sStem(words[index-1]).lower().strip()
        word2 = sStem(words[index]).lower().strip()
        #word3 = sStem(words[index+1]).lower().strip()
        
        word1a = word1 + ' ' + word2
        #word2a = word2 + ' ' + word3
        
        if word2 in lsAllWords: 
            indexWord = lsAllWords.index(word1a)
        else:
            lsAllWords.append(word1a)
            indexWord = len(lsAllWords)-1
        ...
        # if word3 in lsAllWords: 
        #            indexWord = lsAllWords.index(word2a)
        #        else:
        #            lsAllWords.append(word2a)
        #            indexWord = len(lsAllWords)-1
        # 
        #        if indexWord in dWords.keys():
        #            dWords[indexWord]+=1
...
def test(sentence,modelFile,wordsFile,rangeFile,modelDir,subTopicCodes):
    ...
    for scode in subTopicCodes:
        ...
        fTest,lsAllWords = writeFileBigram(sentence,lsAllWords,fTest,scode)
        fTest.close()
...
def classify(text,lsTopics=None):
    ...
    for topic in dJT.values():
        if (lsTopics and topic in lsTopics) or not lsTopics:
            ...
            wordsFile = tempDir + '/words_bigram.txt'
